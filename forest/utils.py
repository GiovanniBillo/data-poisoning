...
def record_results(kettle, brewed_loss, results, args, defs, modelkey, extra_stats=dict()):
    ...
    save_to_table(args.table_path,
                  f'{"_".join(args.net)}_{args.threatmodel}_{args.scenario}_{args.name}', args.dryrun,
                  ...
                  **{f'ARG_{key}': val for key, val in args.__dict__.items()},
                  val_acc_base_clean=_maybe(stats_clean, 'valid_accs_base'),
                  val_acc_target_clean=_maybe(stats_clean, 'valid_accs_target'),
                  val_acc_base_rerun=_maybe(stats_rerun, 'valid_accs_base'),
                  val_acc_target_rerun=_maybe(stats_rerun, 'valid_accs_target'),
                  val_acc_base_reinit=_maybe(stats_results, 'valid_accs_base'),
